...
var figue = function () {
	...
	function kmedians (k, vectors) {
		...
		while (repeat) {
			...
			var newCentroidsArr = new Array(k) ;
			for (var j = 0 ; j < k ; j++) {
				newCentroidsArr[j] = null;
			}
			for (var i = 0 ; i < n ; i++) {
				cluster = assignments[i] ;
				if (newCentroidsArr[cluster] == null) {
					newCentroidsArr[cluster] = new Array();
				}
				newCentroidsArr[cluster].push( vectors[i] );
			}

			var newCentroids = new Array(k) ;
			for (var j = 0 ; j < k ; j++) {
				newCentroids[j] = newCentroidsArr[j].sort(function(a,b){return a-b})[Math.floor(newCentroidsArr[j].length/2)];
			}
